...
def hinge_loss_single(feature_vector: np.ndarray,
                      label: float,
                      theta: np.ndarray,
                      theta_0: float) -> float:
    """
    ...
    Returns:
        ...
        parameters.
    """
    return np.max([0, 1 - label*(theta.T @ feature_vector + theta_0)])



def hinge_loss_full(feature_matrix: np.ndarray,
                    labels: np.ndarray,
                    theta: np.ndarray,
                    theta_0: float) -> float:
    """
    ...
    """
    leeway = labels*(feature_matrix @ theta + theta_0)
    loss_mat = np.array([np.zeros_like(leeway), 1 - leeway])
    loss_arr = np.max(loss_mat, axis=0)
    return np.mean(loss_arr)



def perceptron_single_step_update(feature_vector: np.ndarray,
                                  label: float,
                                  current_theta: np.ndarray,
                                  current_theta_0: float) -> Tuple[np.ndarray, float]:
    """
    ...
    """
    leeway = label*(current_theta.T @ feature_vector + current_theta_0)
    if leeway < 0 or np.isclose(leeway, 0):
...
def perceptron(feature_matrix: np.ndarray, labels: np.ndarray, T: int) -> Tuple[np.ndarray, float]:
    """
    ...
    """
    training_size, n_features = feature_matrix.shape
    theta = np.zeros(n_features)
    theta_0 = 0
    for t in range(T):
        for i in get_order(training_size):
...
def average_perceptron(feature_matrix, labels, T):
    ...
    theta = np.zeros(n_features)
    theta_0 = 0
    theta_iters = np.zeros((T, n_features))
    theta_0_iters = np.zeros(T)
    for t in range(T):
        theta_arr = np.zeros_like(feature_matrix)
        theta_0_arr = np.zeros(training_size)
        for i in get_order(training_size):
            feature_vector = feature_matrix[i, :]
            label = labels[i]
            theta, theta_0 = perceptron_single_step_update(feature_vector, label, theta, theta_0)
            theta_arr[i, :] = theta
            theta_0_arr[i] = theta_0
        theta_iters[t, :] = np.mean(theta_arr, axis=0)
        theta_0_iters[t] = np.mean(theta_0_arr)

    theta = np.mean(theta_iters, axis=0)
    theta_0 = np.mean(theta_0_iters)
    return theta, theta_0



def pegasos_single_step_update(feature_vector: np.ndarray,
                               label: float,
                               L: float,
                               eta: float,
                               theta: np.ndarray,
                               theta_0: float) -> Tuple[np.ndarray, float]:
    """
    Updates the classification parameters `theta` and `theta_0` via a single
